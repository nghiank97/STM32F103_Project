...
void nextion_text_send(u08* id_data, u16 id_len, u08* data, u16 len){
...
void nextion_pic_send(u08* id_data, u16 id_len, u08* data, u16 len){
	if(uart_status == TX_IDLE){
		for(u08 i=0;i<id_len;i++){
			tx_buffer[tx_tail] = id_data[i];
			tx_tail = (tx_tail+1)%LEN_BUFF;
	...
	if (len == 4){
		if (com_arr(data, (u08*)bt_clicked, len)){
			if (len_status == 0){
				...
				LED_ON();
				uart_status = TX_IDLE;
				nextion_text_send((u08*)"t0",2,(u08*)"\"ON\"",4);
			}
			else{
				len_status = 0;
				LED_OFF();
				uart_status = TX_IDLE;
				nextion_text_send((u08*)"t0",2,(u08*)"\"OFF\"",5);
			}
		}
	}
}
...
extern void loop(void){
	...
	uart1_load_package((u08*)"p1.pic=", 7);
	uart1_load_package((u08*)adc_text, adc_len);
	uart1_add_tail();
	
	adc_len = sprintf((char*)adc_text, "\"ADC %2d\"", adc);
	uart1_load_package((u08*)"t0.txt=", 7);
	uart1_load_package((u08*)adc_text, adc_len);
	uart1_add_tail();
